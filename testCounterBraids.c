...
int main(){
	...
	while(fscanf(fp_pkt_tag, "%x\t%x\t%hd\t%hd\t%hhu\t%hhu\t%u\t%d\t%d\n", &pkt.ft.src_ip, &pkt.ft.dst_ip,
		...
		if(pkt.ft.proto != 0x6) continue;
		if(pkt.ft.tag == 1)
			addFlow(flowTable, &index_flowTable, &pkt.ft);
		 updateCounterBraids(hashTable, &pkt.ft, hashTable_Layer2, &index_flowTable_Layer2, flowTable_Layer2);
		num_pkt++;
	}
...
*/
	...
	int num_error = calculateRelatedError(flowTable, index_flowTable, bigFLowTable, &num_error_flow);
